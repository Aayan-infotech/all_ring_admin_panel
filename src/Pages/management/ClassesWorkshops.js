...
// import { PencilSquare, PlusCircle, XCircleFill, CheckCircleFill, Film, FileEarmarkText } from 'react-bootstrap-icons';
...
//       const res = await axios.get('http://18.209.91.97:5010/api/classes', {
...
//     try {
//       const token = localStorage.getItem('adminToken');
//       await axios.patch(`http://18.209.91.97:5010/api/classes/${id}/toggle-status`, {}, {
//         headers: { Authorization: `Bearer ${token}` }
//       });
//       fetchClasses();
//     } catch (err) {
...
//                 <tr key={item._id}>
//                   <td>{idx + 1}</td>
//                   <td><img src={item.image || 'https://via.placeholder.com/40'} alt="img" width="40" height="40" style={{ borderRadius: '4px' }} /></td>
//                   <td>{item.title}</td>
//                   <td>{item.theme}</td>
//                   <td>{item.date}</td>
//                   <td>{item.time}</td>
...
//                   <td>
//                     <ButtonGroup>
//                       <Button variant={item.status === 'active' ? 'danger' : 'success'} size="sm" onClick={() => toggleStatus(item._id)}>
//                         {item.status === 'active' ? <XCircleFill /> : <CheckCircleFill />}
//                       </Button>
//                       <Button variant="warning" size="sm" onClick={() => { setSelectedClass(item); setShowAddForm(true); }}>
//                         <PencilSquare />
//                       </Button>
//                       <Button variant="info" size="sm" onClick={() => { setSelectedClass(item); setShowMediaForm(true); }}>
//                         <Film />
//                       </Button>
//                       <Button variant="secondary" size="sm" onClick={() => { setSelectedClass(item); setShowNotesForm(true); }}>
//                         <FileEarmarkText />
//                       </Button>
//                     </ButtonGroup>
//                   </td>
...
const ClassesWorkshops = () => {
  ...
  const fetchClasses = async () => {
    try {
      ...
      setClasses(res.data.length ? res.data : fallbackData);
    } catch (err) {
      console.error('Error fetching classes, showing fallback data:', err);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      await axios.delete(`http://18.209.91.97:5010/api/classes/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchClasses();
    } catch (err) {
