...
const ClassMediaPage = () => {
  ...
  const fetchClassMedia = async (classId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await axios.get(`http://98.85.246.54:5010/api/mediaAdmin/getAllMedia/${classId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  try {
    const token = localStorage.getItem("adminToken");
    await axios.delete(`http://98.85.246.54:5010/api/mediaAdmin/deleteMedia/${classId}/${mediaId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
