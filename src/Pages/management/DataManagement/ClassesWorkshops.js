...
formData.append('endTime', formatTime(form.sessionEndTime.value || '13:00'));
  ...
  try {
    ...
    const response = await axios.put(
      `http://98.82.228.18:5010/api/AdminClasses/updateClass/${selectedClass._id}`,
      formData,
      {
        headers: { 
...
const fetchQuestions = async (classId) => {
  try {
    ...
    const res = await axios.get(
      `http://98.82.228.18:5010/api/questionaire/getQuestions/${classId}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
const toggleStatus = async (id) => {
  try {
    ...
    const response = await axios.patch(
      `http://98.82.228.18:5010/api/AdminClasses/blockClass/${id}`, 
      {}, 
      {
        headers: { Authorization: `Bearer ${token}` },
      }
...
const handleDelete = async () => {
  try {
    const token = localStorage.getItem('adminToken');
    await axios.delete(`http://98.82.228.18:5010/api/AdminClasses/deleteClass/${itemToDelete}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const handleAddQuestion = async (classId) => {
    try {
      const token = localStorage.getItem('adminToken');
      await axios.post(
        `http://98.82.228.18:5010/api/questionaire/addQuestions/${classId}`,
        { questionText },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleEditQuestion = async (classId, questionId) => {
    try {
      const token = localStorage.getItem('adminToken');
      await axios.put(
        `http://98.82.228.18:5010/api/questionaire/editQuestion/${classId}/${questionId}`,
        { questionText },
        { headers: { Authorization: `Bearer ${token}` } }
      );
