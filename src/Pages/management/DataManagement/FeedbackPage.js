...
const FeedbackPage = () => {
  ...
  const fetchFeedbacks = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/feedback/getAllFeedbacks/${classId}`);
      const feedbacksData = res.data.data || [];
      setFeedbacks(feedbacksData);
      
      // Extract sessionId from the first feedback (assuming all feedbacks have the same sessionId)
      if (feedbacksData.length > 0 && feedbacksData[0].sessionId) {
        setSessionId(feedbacksData[0].sessionId);
        return feedbacksData[0].sessionId;
      }
      return null;
    } catch (err) {
      setError('Failed to load feedbacks');
  ...
  const fetchIncidents = async (sessionId) => {
    try {
      ...
      if (!sessionId) {
        setIncidentError('Session ID is not available');
        setIncidentLoading(false);
        return;
      }
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        setLoading(true);
        // First fetch feedbacks to get sessionId
        const sessionIdFromFeedbacks = await fetchFeedbacks();
        
        // Then fetch questionnaires and incidents
        await fetchQuestionnaires();
        
        if (sessionIdFromFeedbacks) {
          await fetchIncidents(sessionIdFromFeedbacks);
        } else {
          setIncidentError('No session ID found in feedback data');
          setIncidentLoading(false);
        }
      } catch (err) {
                    ...
                    {incidents.map((incident, idx) => (
                      <tr key={incident._id || idx}>
                        ...
                        <td>
                          ...
                          {incident.instructor?.email && (
                            <div className="text-muted small">{incident.instructor.email}</div>
                          )}
                        </td>
                        <td>{incident.description || 'No description'}</td>
                        <td>{incident.createdAt ? new Date(incident.createdAt).toLocaleString() : 'N/A'}</td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
