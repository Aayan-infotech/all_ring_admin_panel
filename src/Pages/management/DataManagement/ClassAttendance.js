...
const ClassAttendance = () => {
  ...
  const handleShowSessions = async (cls) => {
    try {
      ...
      const res = await axios.get(
        `http://52.20.55.193:5010/api/AdminClasses/getClassByIdAdmin/${cls._id}`
      );
...
const handleShowAttendance = async (session) => {
  try {
    ...
    const res = await axios.get(
      `http://52.20.55.193:5010/api/AdminClasses/getSessionAttendence/${selectedClass._id}/${session._id}`
    );
  ...
  //     const res = await axios.get(
  //       `http://52.20.55.193:5010/api/register/getRegistredUserWithAttendence/${selectedClass._id}/${session._id}`
  //     );
