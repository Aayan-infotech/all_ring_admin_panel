...
const AddNotesOffcanvas = ({ show, handleClose, classId }) => {
  ...
  const fetchNotes = async () => {
    try {
      ...
      const response = await axios.get(`http://18.209.91.97:5010/api/notes/getAllNotes/${classId}`);
      setNotes(response.data.data || []); // adjust based on actual response structure
    } catch (error) {
