...
const AddClassOffcanvas = ({ show, handleClose, onSaved }) => {
  ...
  //         axios.get('http://98.85.246.54:5010/api/admin/getRegister/instructor', {
  //           headers: { Authorization: `Bearer ${token}` }
  //         }),
  //         axios.get('http://98.85.246.54:5010/api/location/getAllLocations')
  //       ]);
...
const onSubmit = async (data) => {
  ...
  try {
    const token = localStorage.getItem('adminToken');
    await axios.post('http://98.85.246.54:5010/api/AdminClasses/addClass', formData, {
      headers: { 
