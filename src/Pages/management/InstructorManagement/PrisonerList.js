...
const PrisonerList = () => {
  ...
  const fetchPrisoners = async () => {
    try {
      const res = await axios.get('http://localhost:5010/api/prisoner/getPrisoners');
  ...
  const deletePrisoner = async () => {
    try {
      await axios.delete(`http://localhost:5010/api/prisoner/deletePrisoner/${confirmDelete.id}`);
  ...
  const toggleStatus = async (prisoner) => {
    ...
    try {
      await axios.patch(`http://localhost:5010/api/prisoner/changePrisonerStatus/${prisoner._id}`, {
        status: newStatus
      });
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `http://localhost:5010/api/prisoner/updatePrisoner/${selectedPrisoner._id}`,
        selectedPrisoner
      );
