...
const PrisonerList = () => {
  ...
  const fetchPrisoners = async () => {
    try {
      const res = await axios.get('http://54.205.149.77:5010/api/prisoner/getPrisoners');
  ...
  const deletePrisoner = async () => {
    try {
      await axios.delete(`http://54.205.149.77:5010/api/prisoner/deletePrisoner/${confirmDelete.id}`);
  ...
  const toggleStatus = async (prisoner) => {
    ...
    try {
      await axios.patch(`http://54.205.149.77:5010/api/prisoner/changePrisonerStatus/${prisoner._id}`, {
        status: newStatus
      });
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `http://54.205.149.77:5010/api/prisoner/updatePrisoner/${selectedPrisoner._id}`,
        selectedPrisoner
      );
