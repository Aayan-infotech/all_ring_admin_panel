...
const AssignTeam = () => {
  ...
  const fetchInstructors = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('adminToken');
      let url = `http://54.205.149.77:5010/api/admin/getRegister/instructor?page=${currentPage}&limit=${limit}`;
  ...
  const handleAssignClick = async (instructorId) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://54.205.149.77:5010/api/assignUsers/getUsersByLocation/${instructorId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const assignUsersToInstructor = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('adminToken');
      await axios.post(
        `http://54.205.149.77:5010/api/assignUsers/assignToInstructor/${selectedInstructorId}`,
