...
const [showAddPrisonerOffcanvas, setShowAddPrisonerOffcanvas] = useState(false);
  ...
  const fetchInstructors = async () => {
    try {
      ...
      const res = await axios.get('http://34.206.193.218:5010/api/admin/getRegister/instructor', {
        headers: {
  ...
  const fetchLocations = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      const res = await axios.get('http://34.206.193.218:5010/api/location/getAllLocations', {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
  ...
  const toggleStatus = async (instructor) => {
    ...
    try {
      ...
      await axios.patch(
        `http://34.206.193.218:5010/api/admin/editUserStatus/${instructor._id}`,
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://34.206.193.218:5010/api/admin/changeUserPassword/${selectedInstructor._id}`,
        passwords,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://34.206.193.218:5010/api/auth/update-user/${selectedInstructor._id}`,
        formDataToSend,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
