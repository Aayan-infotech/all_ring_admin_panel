...
const AddPrisonerOffcanvas = ({ show, handleClose, instructorId, locations, preSelectedLocation }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post('http://98.85.246.54:5010/api/prisoner/addPrisoner', payload);
  ...
  const fetchPrisoners = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`http://98.85.246.54:5010/api/prisoner/prisonersByInstructor/${instructorId}`);
