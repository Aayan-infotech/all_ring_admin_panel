...
const AddPrisonerOffcanvas = ({ show, handleClose, instructorId, locations, preSelectedLocation }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post('http://localhost:5010/api/prisoner/addPrisoner', payload);
  ...
  const fetchPrisoners = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5010/api/prisoner/prisonersByInstructor/${instructorId}`);
