...
const AddPrisonerOffcanvas = ({ show, handleClose, instructorId, locations, preSelectedLocation }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post('http://54.205.149.77:5010/api/prisoner/addPrisoner', payload);
  ...
  const fetchPrisoners = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`http://54.205.149.77:5010/api/prisoner/prisonersByInstructor/${instructorId}`);
